...
#            rather than np.zeros(dim) = (0°, 0°), which lies far outside the
#            North Sea.  If the entire initial population is infeasible, leaders
#            stay in-region.
# =============================================================================
...
from lcoe import compute_lcoe, lcoe_breakdown
...
# =============================================================================
# GWO IMPLEMENTATION — adapted for LCOE wind farm siting
# =============================================================================

def GWO_implementation(population_size: int, max_evals: int):
    """
    Grey Wolf Optimiser for LCOE minimisation.
    ...
    # --- Initialise alpha, beta, delta (three best wolves) ---
    # Fix 1: initialise positions to random points within bounds 
    Alpha_pos = np.random.uniform(lb, ub)
    Alpha_score = float("inf")
    Beta_pos = np.random.uniform(lb, ub)
    Beta_score = float("inf")
    Delta_pos = np.random.uniform(lb, ub)
    ...
    # --- Main loop ---
    # Calculate max iterations from remaining budget
    # Each iteration evaluates population_size agents
    ...
    # Build a simple archive from the three leaders
    archive = []
    for pos, score in [(Alpha_pos, Alpha_score),
                       (Beta_pos, Beta_score),
                       (Delta_pos, Delta_score)]:
        if np.isfinite(score):
            archive.append({"x": pos.copy(), "f": score})
...
def plot_north_sea(
    ...
    if archive:
        ...
        ax.scatter(
            ...
            zorder=4, label="Top wolves (α, β, δ)",
            transform=proj,
        )
    ...
    ax.set_title(
        "GWO — Offshore Wind Farm LCOE Optimisation (North Sea, incl. wind)",
        fontsize=14,
    )
...
def plot_convergence(cost_history: list) -> None:
    ...
    plt.title("GWO Convergence — LCOE Minimisation")
...
if __name__ == "__main__":
    ...
    max_evals = 5000
    independentRuns = 20 # ≥10 recommended for meaningful summary statistics
    populationSize = 30
    ...
    for run in range(independentRuns):
        ...
        (
            ...
            search_costs,
            archive,
        ) = GWO_implementation(population_size=populationSize, max_evals=max_evals)
    ...
    if best_loc is not None:
        ...
        if independentRuns >= 2:
            ...
            fig.suptitle(
                f"GWO Multi-Run Summary ({independentRuns} runs, "
        ...
        if independentRuns >= 2:
            ...
            plt.xlabel("Objective function evaluations")
            plt.ylabel("Best LCOE (£/MWh)")
            plt.title("GWO Convergence — All Runs Overlaid")
